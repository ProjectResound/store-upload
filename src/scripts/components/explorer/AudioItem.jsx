...
export default ({ audioItem, isAppending, index }) => {
  ...
  return (
    <Link
      to={generateUrl(audioItem)}
      className={`explorer__table__audio-item row ${animationClass}`}
    >
      <div className="explorer__table__audio-item__property explorer__table__audio-item-title col s3">
        {audioItem.title}
      </div>
      <div className="explorer__table__audio-item__property col s3">
        {audioItem.filename}
      </div>
